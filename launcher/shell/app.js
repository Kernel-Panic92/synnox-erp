...
// ── Usage tracking for dynamic quick access ──
...
function getTopModules(limit = 5) {
  const usage = JSON.parse(localStorage.getItem('module_usage') || '{}');
...
async function cargarSidebar() {
  // User info
  const userInfoEl = document.getElementById('lsb-user-info');
  if (userInfoEl) {
    userInfoEl.innerHTML = `
      <div class="lsb-name">${esc(user?.nombre || '')}</div>
      <div class="lsb-role">${esc(user?.rol || '')}</div>
      <div class="lsb-email">${esc(user?.email || '')}</div>
    `;
  }

  // Quick access (dynamic based on usage)
  const qaEl = document.getElementById('lsb-quick-access');
  if (qaEl) {
    const topMods = getTopModules(5);
    const modulos = user?.rol === 'admin' ? MODULOS_FIJOS : MODULOS_FIJOS.filter(m => user?.modulos?.includes(m.id));
    const sorted = topMods.length > 0
      ? topMods.map(t => modulos.find(m => m.id === t.id)).filter(Boolean)
      : modulos.slice(0, 5);
    qaEl.innerHTML = sorted.map(m => `
      <a href="${window.location.origin + m.ruta}" target="_blank" class="lsb-link" onclick="trackModuleVisit('${m.id}')">
        <span class="lsb-icon">${m.icon}</span>
        <span>${m.nombre}</span>
      </a>
    `).join('');
  }
  ...
  // System status
  const statusEl = document.getElementById('lsb-status');
  if (statusEl) {
    try {
      const res = await fetch('/api/admin/health', { headers: { 'Authorization': 'Bearer ' + jwtToken } });
      const data = await res.json();
      const mods = data.modules || [];
      statusEl.innerHTML = mods.map(m => `
        <div class="lsb-status-item">
          <span>${esc(m.id)}</span>
          <span class="lsb-status-dot ${m.status === 'ok' ? 'online' : 'offline'}"></span>
        </div>
      `).join('') || '<div style="color:var(--muted)">Sin datos</div>';
    } catch { statusEl.innerHTML = '<div style="color:var(--muted)">Error</div>'; }
  }

  // Pending tasks
  const pendingEl = document.getElementById('lsb-pending');
  if (pendingEl) {
    try {
      const [facturas, registros] = await Promise.allSettled([
      ...
      const items = [];
      if (facturas.status === 'fulfilled' && facturas.value?.total > 0) items.push({ icon: '📄', text: `${facturas.value.total} factura(s) por revisar`, link: '/proveedores/#pendientes' });
      if (registros.status === 'fulfilled' && registros.value?.total > 0) items.push({ icon: '📝', text: `${registros.value.total} registro(s) pendiente(s)`, link: '/nomina/#registros' });
      pendingEl.innerHTML = items.length ? items.map(i => `
        <a href="${i.link}" target="_blank" class="lsb-link">
          <span class="lsb-icon">${i.icon}</span>
          <span>${i.text}</span>
        </a>
      `).join('') : '<div style="color:var(--muted)">✅ Sin pendientes</div>';
    } catch { pendingEl.innerHTML = '<div style="color:var(--muted)">Error</div>'; }
  }

  // Activity
  const activityEl = document.getElementById('lsb-activity');
  if (activityEl) {
    try {
      const res = await fetch('/api/admin/login-logs', { headers: { 'Authorization': 'Bearer ' + jwtToken } });
      const data = await res.json();
      const logs = (data.logs || []).slice(0, 5);
      activityEl.innerHTML = logs.length ? logs.map(l => {
        const d = new Date(l.fecha || l.timestamp);
        const time = d.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' });
        return `
          <div class="lsb-activity-item">
            <span>${l.exitoso ? '🔑' : '🚫'}</span>
            <span style="flex:1">${esc(l.email || '—')}</span>
            <span class="lsb-activity-time">${time}</span>
          </div>
        `;
      }).join('') : '<div style="color:var(--muted)">Sin actividad</div>';
    } catch { activityEl.innerHTML = '<div style="color:var(--muted)">Error</div>'; }
  }
}

...
module.exports = function createUsuariosRouter({
  ...
  rolTienePermiso, enviarCorreo, middlewares, APP_NAME
}) {
  ...
  router.post('/', soloAdmin, async (req, res) => {
    ...
    try {
      ...
      if (cfg.smtp_host) {
        ...
        await enviarCorreo(email, `Bienvenido a ${APP_NAME} - Configura tu contraseña`,
          `Hola ${nombre},\n\nTu cuenta ha sido creada en ${APP_NAME}.\n\nCorreo: ${email}\nRol: ${rolTxt}\nSede: ${sede}\n\nPor favor configura tu contraseña haciendo clic en el siguiente enlace (válido por 7 días):\n${enlace}\n\nSaludos,\nEquipo ${APP_NAME}`
        );
      }
  ...
  router.post('/:id/reset-password', soloAdmin, async (req, res) => {
    ...
    try {
      await enviarCorreo(usuario.email, `Restablecer contraseña - ${APP_NAME}`, cuerpo);
      res.json({ ok: true });
    } catch (e) {

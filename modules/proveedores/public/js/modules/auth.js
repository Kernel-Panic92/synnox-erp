...
function buildNav(){
  ...
  for(const sec of SECS){
    ...
    h+=`<div class="sidebar-section-title" style="font-size:9px;color:var(--muted);letter-spacing:.1em;text-transform:uppercase;padding:10px 24px 4px;margin-top:4px">${sec.l}</div>`;
    for(const n of items)h+=`<button class="nav-item" id="nv-${n.id}" data-page="${n.id}" onclick="goNav('${n.id}')" title="${n.l}" data-tooltip="${n.l}" aria-label="${n.l}"><span class="icon">${n.i}</span> <span class="nav-text">${n.l}</span>${n.badge?`<span class="badge" style="font-size:10px;padding:2px 6px;background:${n.w?'rgba(251,191,36,.15)':'rgba(79,142,247,.15)'};color:${n.w?'var(--warning)':'var(--accent)'}" id="nb-${n.badge}">0</span>`:''}</button>`;
  }

...
async function checkSyncStatus(){
  try{
    ...
    if(r.sincronizando){
      // Show restart button if stuck (0 progress) or if no progress after some time
      ...
      return{sincronizando:true,bar:`<div style="background:rgba(79,142,247,.1);border:1px solid rgba(79,142,247,.3);border-radius:12px;padding:16px;margin-bottom:20px">
        ...
        <div style="margin-top:10px"><button class="btn btn-danger btn-sm" onclick="reiniciarSync()">🔄 Reiniciar</button></div>
      </div>`};
    }else{

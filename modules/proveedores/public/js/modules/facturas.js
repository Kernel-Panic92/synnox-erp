...
async function verAcuse(id){
  try {
    const resp = await fetch(`${BASE}/api/facturas/${id}/acuse`);
    if (!resp.ok) {
      const err = await resp.json().catch(()=>({error:'Error'}));
      toast(err.error || 'Error cargando acuse', 'error');
      return;
    }
    const text = await resp.text();
    $('mroot').innerHTML=`<div class="modal-overlay open" onclick="if(event.target===this){closeM();}">
      <div class="modal" style="width:90vw;max-width:900px;height:85vh;display:flex;flex-direction:column">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
          <span style="font-family:var(--font-head);font-size:16px;font-weight:700">✉️ Acuse de Recibo DIAN</span>
          <div style="display:flex;gap:8px">
            <button class="btn btn-secondary btn-sm" onclick="descargarAcuse('${id}')">⬇️ Descargar</button>
            <button class="btn btn-secondary btn-sm" onclick="closeM()">✕ Cerrar</button>
          </div>
        </div>
        <pre style="flex:1;overflow:auto;padding:16px;background:var(--surface2);border-radius:8px;font-size:12px;font-family:monospace;white-space:pre-wrap">${esc(text)}</pre>
    ...
    toast('Error cargando acuse', 'error');
  }
}

function descargarAcuse(id){
  const a=document.createElement('a');
  a.href=`${BASE}/api/facturas/${id}/acuse`;
  a.download='acuse.xml';
  document.body.appendChild(a);
  a.click();
  a.remove();
}
...
async function rFacturas(filtro){
  ...
  params.set('limit',fBusqueda.limit||'100');
  ...
  $('content').innerHTML=`
    ...
    <div class="tbl">
      <div class="tbl-head" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
        <div class="tbl-title">${f.total||all.length} factura(s)</div>
        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
          <div id="bulk-actions" style="display:none;gap:8px;align-items:center">
            ...
            <button class="btn btn-danger btn-sm" onclick="eliminarSeleccionadas()">🗑️ Eliminar</button>
            ${hayFiltros?`<button class="btn btn-danger btn-sm" onclick="eliminarFiltradas()">🗑️ Todas (${f.total||0})</button>`:''}
          </div>
          <div style="display:flex;align-items:center;gap:6px;font-size:13px;color:var(--muted)">
            <span>Mostrar</span>
            <select id="ff-limit" onchange="fBusqueda.limit=this.value;guardarFiltros();rFacturas()" style="padding:4px 8px;border-radius:6px;border:1px solid var(--border);background:var(--surface);color:var(--text);font-size:13px">
              <option value="25" ${fBusqueda.limit==='25'?'selected':''}>25</option>
              <option value="50" ${fBusqueda.limit==='50'?'selected':''}>50</option>
              <option value="100" ${(fBusqueda.limit||'100')==='100'?'selected':''}>100</option>
              <option value="500" ${fBusqueda.limit==='500'?'selected':''}>500</option>
            </select>
            <span>registros</span>
          </div>
        </div>
      </div>
      <table><thead><tr>${isAdmin?'<th style="width:40px"><input type="checkbox" id="select-all" onchange="toggleSelectAll(this)"></th>':''}<th># Factura</th><th>Centro</th><th>Proveedor</th><th>Categoría</th><th>Valor</th><th>Estado</th><th>Recibida</th><th>Acciones</th></tr></thead>
      <tbody>${all.length?all.map(f=>`<tr onclick="abrirF('${f.id}')">${isAdmin?`<td style="width:40px"><input type="checkbox" class="factura-cb" value="${f.id}" onclick="event.stopPropagation();updateBulkActions()"></td>`:''}<td class="mono" data-label="Factura">${esc(f.numero_factura)}</td><td data-label="Centro" style="font-size:12px;color:var(--muted)">${esc(f.centro_operacion_nombre||'—')}</td><td data-label="Proveedor" style="font-weight:500">${esc(f.proveedor_nombre||f.nombre_emisor||'—')}</td><td data-label="Categoría">${ctag(f.categoria_color,f.categoria_nombre)}</td><td data-label="Valor" style="font-weight:500">${fmt(f.valor_total||f.valor||0)}</td><td data-label="Estado">${bdg(f.estado)}</td><td data-label="Recibida" style="color:var(--muted);font-size:12px">${fdatetime(f.recibida_en)}</td><td data-label="Acciones" style="white-space:nowrap">${f.archivo_pdf?`<button class="btn btn-sm btn-secondary" onclick="event.stopPropagation();verPdf('${f.id}')" title="Ver PDF">📄</button>`:''}${f.archivo_xml?`<button class="btn btn-sm btn-secondary" onclick="event.stopPropagation();verXml('${f.id}')" title="Ver XML">📋</button>`:''}${isAdmin?`<button class="btn btn-sm btn-danger" onclick="event.stopPropagation();delFactura('${f.id}','${esc(f.numero_factura)}')" title="Eliminar">🗑️</button>`:''}</td></tr>`).join(''):'<tr><td colspan="9" class="empty">Sin facturas</td></tr>'}</tbody></table>
    </div>`;
  refreshBadges();
}
...
async function abrirF(id){
  ...
  showM(`Factura ${f.numero_factura}`,`
    ...
    <div class="modal-footer">${f.archivo_pdf?`<button onclick="verPdf('${id}')" class="btn btn-secondary btn-sm">📄 Ver PDF</button>`:''}${f.archivo_xml?`<button onclick="verXml('${id}')" class="btn btn-secondary btn-sm">📋 Ver XML</button>`:''}${f.archivo_acuse?`<button onclick="verAcuse('${id}')" class="btn btn-secondary btn-sm">✉️ Ver Acuse</button>`:''}<button class="btn btn-secondary btn-sm" onclick="closeM()">Cerrar</button>${acc.join('')}</div>`,640);
}

    ...
    overlay.innerHTML = `
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:32px;width:340px;text-align:center">
        ...
        <div style="display:flex;gap:12px;justify-content:center">
          <button class="btn" style="background:var(--surface2);color:var(--text);min-width:100px" onclick="this.closest('.modal-overlay').remove();window._confirmResolve(false)">Cancelar</button>
          <button class="btn btn-danger" style="min-width:100px" onclick="this.closest('.modal-overlay').remove();window._confirmResolve(true)">Confirmar</button>
        </div>
      </div>`;
